...
fn test_inference_context_symbol_operations() {
    ...
    assert_eq!(context.get_symbol_type("x"), Some(&Type::Concrete("i32".to_string())));
    assert_eq!(context.get_symbol_type("y"), Some(&Type::Generic("T".to_string(), vec![])));
...
fn test_inference_context_type_cache() {
    ...
    assert_eq!(context.get_cached_type(1), Some(&Type::Concrete("String".to_string())));
