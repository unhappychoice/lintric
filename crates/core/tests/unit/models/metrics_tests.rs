use lintric_core::models::{LineMetrics, AnalysisResult, OverallAnalysisReport};
...
fn test_line_metrics_clone() {
    ...
    assert_eq!(original.dependency_distance_cost, cloned.dependency_distance_cost);
    assert_eq!(original.depth, cloned.depth);
    assert_eq!(original.transitive_dependencies, cloned.transitive_dependencies);
...
fn test_analysis_result_clone() {
    ...
    assert_eq!(original.overall_complexity_score, cloned.overall_complexity_score);
}
