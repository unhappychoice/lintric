use lintric_core::models::{Position, Usage, UsageKind};
...
fn test_usage_new_simple() {
    ...
    let usage = Usage::new_simple("test_usage".to_string(), position.clone(), UsageKind::Read);
...
fn test_usage_new_call_expression() {
    ...
    let mut parser = setup_rust_parser();
    let tree = parser
        .parse("fn main() { function_call(); }", None)
        .unwrap();
