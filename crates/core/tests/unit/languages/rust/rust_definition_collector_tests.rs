...
fn another_function(param: i32) -> i32 {
    ...
    let func_defs: Vec<_> = definitions
        .iter()
    ...
    assert!(
        function_names.contains(&&"test_function".to_string()),
        "Should find test_function"
    );
    assert!(
        function_names.contains(&&"another_function".to_string()),
...
struct TupleStruct(i32, String);
    ...
    assert!(
        struct_names.contains(&&"TestStruct".to_string()),
        "Should find TestStruct"
    );
    assert!(
        struct_names.contains(&&"EmptyStruct".to_string()),
        "Should find EmptyStruct"
    );
    assert!(
        struct_names.contains(&&"TupleStruct".to_string()),
        "Should find TupleStruct"
    );
}
...
fn main() {
    ...
    assert!(
        ...
        "Should find variable x"
    );
    assert!(
        var_names.contains(&&"y".to_string()),
        "Should find variable y"
    );
}
...
enum Message {
    ...
    assert!(
        enum_names.contains(&&"Color".to_string()),
        "Should find Color enum"
    );
    assert!(
        enum_names.contains(&&"Message".to_string()),
        "Should find Message enum"
    );
}
...
impl Point {
    ...
    assert!(
        method_names.contains(&&"distance_from_origin".to_string()),
...
pub mod network {
    ...
    let mod_defs: Vec<_> = definitions
        .iter()
    ...
    assert!(
        mod_names.contains(&&"utils".to_string()),
        "Should find utils module"
    );
    assert!(
        mod_names.contains(&&"network".to_string()),
        "Should find network module"
    );
    assert!(
        mod_names.contains(&&"tcp".to_string()),
        "Should find tcp module"
    );
}
